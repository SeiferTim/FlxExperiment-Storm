...
package org.flixel.plugin.photonstorm 
{
	...
	public class FlxWeapon 
	{
		...
		public var onDeathCallback:Function;
		...
		public function addPixelBullets(quantity:uint, width:int = 2, height:int = 2, color:uint = 0xffffffff):void
		{
			var startLength:Number = group.length;
			group.maxSize += quantity;
			for (var b:uint = 0; b < quantity; b++)
			{
				var tempBullet:Bullet = new Bullet(this, b+startLength);
				tempBullet.makeGraphic(width, height, color);
				group.add(tempBullet);
			}
		}
		 ...
		 */
		private function runFire(method:uint, x:Number = 0, y:Number = 0, target:FlxSprite = null, angle:int = 0):Boolean
		{
			...
			var launchX:Number = positionOffset.x;
			var launchY:Number = positionOffset.y;
		 ...
		 * @param	offsetY		When the bullet is fired if you need to offset it on the y axis, for example to line it up with the "nose" of a space ship, set the amount here (positive or negative)
		 */
		public function setFiringPosition(x:Number, y:Number, offsetX:int = 0, offsetY:int = 0):void
		{
		...
		public function setDeathCallback(callback:Function = null):void
		{
		...
		{
			// ???
			//setBulletBounds(new FlxRect( FlxG.camera.scroll.x-128, FlxG.camera.scroll.y-128, FlxG.width+128, FlxG.height+128));
		}
		...
		public function destroy():void
		{
			...
			if (onPostFireSound)
			{
				onPostFireSound.destroy();
				onPostFireSound = null;
			}
			if(magazineSwapSound)
			{
				magazineSwapSound.destroy();
		...
		public function get rotateToAngle():Boolean 
		{
			return _rotateToAngle;
		}
		
		public function set rotateToAngle(value:Boolean):void 
		{
			_rotateToAngle = value;
		}
	}
